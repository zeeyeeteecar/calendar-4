...
export default function SearchMember({
  ...
  return (
    <div className="flex-col w-full h-full border-green-400 border-0">
      <header className="w-full border-2 text-center p-3 text-lg">
        SearchMember
      </header>

      <form
        action={handle_SearchMember}
        className="space-x-3 m-2 flex flex-row border-0 "
      >
        <input
          type="text"
          name="Fname"
      ...
      <form
        ...
        className="w-full h-[750px] border-0 border-red-500 p-2"
      >
        <div className="flex flex-col flex-1 border-0 p-2 h-full w-full  ">
          <div className="w-full h-full p-1 flex flex-col justify-center items-center border-0  ">
            <div className="w-full h-full border-2 overflow-visible overflow-y-auto">
              {globe_MemberSearchResult &&
                globe_MemberSearchResult.map((member: any, key: number) => {
                  return (
                    <div key={key}>
                      <SearchMemberRow member={member} />
                    </div>
                  );
                })}
            </div>
